...
| Criterion | State |
|---|---|
| Secret-keeping (whisper never leaks to non-audience) | Partial — projection-level secrecy (phase 2) plus perception now enforces who-could-sense: a whisper reaches only close in-zone entities and produces no overhear for anyone else (phase 3). Full criterion needs context assembly (4) to wire perception into belief-store construction. |
