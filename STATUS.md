...
| Criterion | State |
|---|---|
| Secret-keeping (whisper never leaks to non-audience) | Partial — projection-level secrecy (phase 2) plus perception enforces who-could-sense: a whisper reaches only close in-zone entities, with no content/identity leak in overhears (phase 3, stress-tested). One residual metadata side-channel: a non-audience POV can infer *that* hidden events occurred from global `sequence` gaps (D-013, fix in phase 4). Full criterion also needs context assembly (4). |
