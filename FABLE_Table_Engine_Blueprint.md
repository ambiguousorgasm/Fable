...
- **Rules engine.** Deterministic code implementing FABLE's mechanics (the 3d6 roll and result bands, TN/Exposure/Effect, the Trade and Ledger economies, Truths, Clocks, Fronts, Edge, Stress/Scars; combat and the other Modes are configurations of these surfaces, not separate subsystems). The single authority for mechanical outcomes. Specified in `fable_engine.md` (Engine Schema v4).
- **Dice service.** Logged, auditable randomness. No outcome a model claims is real unless it came from here.
- **Stakes gate.** The adjudicator's first question on any declared action: *are there stakes?* If no risk, no opposition, no uncertain outcome — no roll. This is FABLE's **Exit Check** plus the *no-empty-rolls* rule (`fable_engine.md` §11, §5; invariant 11). Most lifelike texture comes from stakes-free actions that still ripple through perception and disposition.
