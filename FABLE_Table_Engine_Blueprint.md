...
- **Rules engine.** Deterministic code implementing FABLE's mechanics (the 3d6 roll and result bands, TN/Exposure/Effect, the Trade and Ledger economies, Truths, Clocks, Fronts, Edge, Stress/Scars; combat and the other Modes are configurations of these surfaces, not separate subsystems). The single authority for mechanical outcomes. Specified in `uploads/FABLE_Engine_Schema_v6.md`.
- **Dice service.** Logged, auditable randomness. No outcome a model claims is real unless it came from here.
- **Stakes gate.** The adjudicator's first question on any declared action: *are there stakes?* If no risk, no opposition, no uncertain outcome — no roll. This is FABLE's **Exit Check** plus the *no-empty-rolls* rule (`uploads/FABLE_Engine_Schema_v6.md` §11, §5; invariant 11). Most lifelike texture comes from stakes-free actions that still ripple through perception and disposition.
...
- **Character sheet:** FABLE character anatomy (Concept, Skills 0–4, Traits, Bonds, Drive, Question, Gear) plus the Stress track, Scars, and Edge. Mechanical truth, owned by the core (`uploads/FABLE_Engine_Schema_v6.md` §4, §13–14).
...
The FABLE ruleset — specified in `uploads/FABLE_Engine_Schema_v6.md`: the 3d6 curve, TN/Exposure/Effect, the Trade and Ledger economies, Truths, Clocks and Fronts, Edge, and EV-audited Modes — instantiates the **rules engine** slot. `uploads/FABLE_Engine_Schema_v6.md` is authoritative for *rules mechanics*; this blueprint remains authoritative for *architecture* (the two are orthogonal — CORE defines the system around the ruleset, not the ruleset itself). Existing code — a FastAPI + SQLite + React/TypeScript engine, and a Python playtest harness with director-pattern turn routing (`[SPOTLIGHT:]` tagging) and per-character TTS — is the implementation substrate. This blueprint is the target those iterate *toward*; where current behavior conflicts with it, the blueprint is the authority unless consciously revised.
